...
const page = () => {
  return (
    <div>
      <div className="w-full bg-[#212121] min-h-screen flex items-center justify-between">
        {/* Text Section */}

        <div className="w-3/5 pl-10 lg:pl-28 pt-20 flex flex-col gap-y-10">
          <div>
            <h2 className="text-white text-lg lg:text-xl font-medium mb-4">Contact Us</h2>
            <h1 className="text-4xl lg:text-5xl font-semibold pb-4 bg-clip-text text-transparent"
              style={{
                backgroundImage:
                  "linear-gradient(90deg, #F5F5F5 0, #B6B5B5 50%, #F5F5F5 100%)",
                WebkitTextFillColor: "transparent",
              }} >
              Have an Inquiry to make?
            </h1>

            <p className='text-[#A3A3A3] fontPoppins font-regular text-base lg:text-xl'>
              Message our admin and we’ll forward your information to our Management Team!</p>
          </div>
          ...
          <button className=" bg-[#0FB02D] w-[35vw] lg:w-[20vw] h-[8vw] lg:h-[5.5vw] rounded-2xl lg:rounded-3xl text-[2.8vw] lg:text-[1.7vw] pr-6 text-white  fontPoppins font-semibold flex justify-center items-center gap-3 transition-all hover:opacity-75 hover:-translate-y-2" 
            onClick={() => window.location.href = 'https://api.whatsapp.com/send?phone=6281389799363&text=Halo%20admin%20Tuna%20Toro!'}>
          ...
          <p className="text-white text-base lg:text-xl fontPoppins font-regular leading-relaxed mb-4">
            For Business Inquiries please <span className="font-bold">message (+62) 813-8979-9363</span>
          </p>

        </div>

        {/* Image Section */}
        <div className="w-2/5 mr-20 flex items-center justify-end">
          <Image
            src="/nav/logo.png"
            alt="Tuna Toro Dish"
            width={300}
            height={300}
            className="max-w-[68%] rounded-lg"
          />
        </div>
      </div>
    </div>
